...
export function load(sha: string, fileName: string, names: NamedAsset[]): Promise<Loaded> {
	const already = loaded.get(sha);
	if (already) return already;

	const promise = read(sha, fileName, names).catch(() => placeholder());
	loaded.set(sha, promise);
	void promise.then((mesh) => {
		measured.update((sizes) => new Map(sizes).set(sha, mesh.size));
	});
	return promise;
}

async function read(sha: string, fileName: string, names: NamedAsset[]): Promise<Loaded> {
	const manager = managerFor(names);
	const url = assetUrl(sha);
