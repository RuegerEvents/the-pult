...
fn generate_catalogue(workspace: &PathBuf) -> Result<()> {
    ...
    let mut lines: Vec<String> = vec![
        ...
        String::new(),
        "import type { StockShape } from \"./StockShape.js\";".into(),
        "import type { SceneObjectKind } from \"./SceneObjectKind.js\";".into(),
        String::new(),
        "/** One piece the console can draw without being given a mesh. */".into(),
        "export type StockPiece = {".into(),
        "\tid: string;".into(),
        "\ttitle: string;".into(),
        "\tshape: StockShape;".into(),
        "\tkind: SceneObjectKind;".into(),
        "\t/** Metres, along X, up Y and along Z. */".into(),
        "\tsize: { x: number; y: number; z: number };".into(),
        "};".into(),
        String::new(),
        "/** Everything the console can draw for itself. */".into(),
        "export const CATALOGUE: StockPiece[] = [".into(),
    ];
    for piece in CATALOGUE {
        lines.push("\t{".into());
        lines.push(format!("\t\tid: {:?},", piece.id));
        lines.push(format!("\t\ttitle: {:?},", piece.title));
        lines.push(format!("\t\tshape: {:?},", format!("{:?}", piece.shape)));
        lines.push(format!("\t\tkind: {:?},", format!("{:?}", piece.kind)));
        lines.push(format!(
            "\t\tsize: {{ x: {}, y: {}, z: {} }}",
