...
/// Write the stock catalogue out as TypeScript data.
///
/// ts-rs exports *types*, and what the browser needs here is the table itself — the
/// ids, the titles and the dimensions. Two copies of that would be two answers to
/// how long an `f34-2m` is, and the one the browser drew would be the one an
/// operator saw. So the Rust table is the source and this is a reading of it, like
/// everything else in `generated/`.
fn generate_catalogue(workspace: &PathBuf) -> Result<()> {
    use pult_schema::types::catalogue::CATALOGUE;

    let out_dir = workspace.join("frontend/src/lib/generated");
    fs::create_dir_all(&out_dir)?;

    // The table itself, through serde. Written this way rather than field by field so
    // that what lands here is what `StockPiece`'s own `Serialize` says — the shape
    // ts-rs already generated the type for. Two spellings of a `PropertyKind` would be
    // a sheet that offers a control the geometry never reads.
    let table = serde_json::to_string_pretty(CATALOGUE)?;

    let lines: Vec<String> = vec![
        "// Auto-generated by pult-codegen — do not edit manually.".into(),
        "// Run: cargo run -p pult-codegen -- generate".into(),
        String::new(),
        "import type { StockPiece } from \"./StockPiece.js\";".into(),
        String::new(),
        "export type { StockPiece };".into(),
        String::new(),
        "/**".into(),
        " * Everything the console can draw for itself.".into(),
        " *".into(),
        " * Dimensions, where each piece bolts to the next, the lines a clamp can go".into(),
        " * round, and what each one asks about itself. The *geometry* is not here and".into(),
        " * is not the browser's: `/stock/{id}.glb` serves what `pult-schema`'s own".into(),
        " * `stock` module generates, so the bytes drawn are the bytes exported.".into(),
        " */".into(),
        format!("export const CATALOGUE: StockPiece[] = {table};"),
        String::new(),
        "/**".into(),
        " * One piece by the id a scene object carries.".into(),
        " *".into(),
        " * `undefined` for a name this build has never heard of, which draws as nothing".into(),
        " * rather than failing — the rule a layout already follows for a panel id it does".into(),
        " * not know.".into(),
        " */".into(),
        "export function piece(id: string | null | undefined): StockPiece | undefined {".into(),
        "\treturn id ? CATALOGUE.find((entry) => entry.id === id) : undefined;".into(),
        "}".into(),
    ];

    let path = out_dir.join("catalogue.ts");
    fs::write(&path, lines.join("\n") + "\n")?;
    eprintln!("✓ Generated the stock catalogue → {}", path.display());
    Ok(())
}
