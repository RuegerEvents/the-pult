...
/// The WIT package version this backend implements. A manifest asking for a
/// different API is reported as failed rather than loaded wrongly.
...
impl PluginManifest {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if self.plugin.api != API_VERSION {
            return Err(format!(
                "built against plugin API {}, this station speaks {}",
                self.plugin.api, API_VERSION
            ));
        }
