...
use pult_schema::types::{
    effect::{
        Curve, Direction, Easing, EffectSource, EffectSpec, Rate, RunningEffect, Shape, Spread,
        Step,
    },
...
fn a_one_hertz_sine_peaks_at_250_ms_and_troughs_at_750() {
    ...
    assert_close(float(&value_at(&effect, 250)), 1.0, "peak");
    assert_close(float(&value_at(&effect, 750)), 0.0, "trough");
    assert_close(float(&value_at(&effect, 0)), 0.5, "starts halfway");
}
...
fn two_phases_half_a_cycle_apart_are_mirror_images() {
    ...
    for ms in [0, 125, 250, 375, 500] {
        let a = float(&value_at(&first, ms));
        let b = float(&value_at(&second, ms));
...
fn low_and_high_scale_the_shape_without_changing_it() {
    ...
    assert_close(float(&value_at(&effect, 250)), 0.6, "peaks at high");
    assert_close(float(&value_at(&effect, 750)), 0.2, "troughs at low");
    assert_close(float(&value_at(&effect, 0)), 0.4, "halfway between");
}
...
fn a_boolean_under_a_square_wave_spends_half_the_cycle_on() {
    ...
    assert_eq!(value_at(&effect, 0), ParameterValue::Bool(true));
    assert_eq!(value_at(&effect, 250), ParameterValue::Bool(true));
    assert_eq!(value_at(&effect, 500), ParameterValue::Bool(false));
    assert_eq!(value_at(&effect, 750), ParameterValue::Bool(false));
}
...
fn a_hard_chase_shows_each_step_from_where_it_starts() {
    ...
    assert_eq!(value_at(&chase, 0), rgb(1.0, 0.0, 0.0), "red");
    assert_eq!(value_at(&chase, 200), rgb(1.0, 0.0, 0.0), "still red");
    assert_eq!(value_at(&chase, 400), rgb(0.0, 1.0, 0.0), "green");
    assert_eq!(value_at(&chase, 700), rgb(0.0, 0.0, 1.0), "blue");
    assert_eq!(value_at(&chase, 1_000), rgb(1.0, 0.0, 0.0), "round to red");
}
...
fn a_linear_chase_crossfades_between_its_steps() {
    ...
    let ParameterValue::Color { r, g, b } = value_at(&chase, 1_000 / 6) else { panic!("a colour") };
...
fn the_last_step_wraps_round_to_the_first() {
    let chase = a_chase(Easing::Linear);
    let ParameterValue::Color { r, g, b } = value_at(&chase, 5_000 / 6) else { panic!("a colour") };
...
fn a_step_list_renders_the_same_however_it_was_ordered() {
    ...
    for ms in [0, 200, 400, 700, 900] {
        assert_eq!(value_at(&ordered, ms), value_at(&shuffled, ms), "at {ms} ms");
    }
}
...
fn a_step_list_with_no_steps_holds_the_bottom_of_its_range() {
    ...
    assert_eq!(value_at(&empty, 500), ParameterValue::Float(0.25));
}
